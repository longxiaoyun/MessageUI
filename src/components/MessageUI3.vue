            ...
            <div class="chat-box">
              <template v-for="item in messageRecords">
                <div :class="item.type === 1 ? 'item left':'item right'" >
                  ...
                  <div class="message">
                    <p>{{ item.content }}</p>
    <!--                <p style="clear: both">2019-08-29 11:00:00</p>-->
                  </div>
                </div>
              </template>
            </div>
  ...
  methods: {
    ...
    sendMsg() {
      ...
      this.messageRecords.push({
        ...
        fromName: this.curentUserName,
        toName:this.targetUserName,
    ...
    getItem(subitem) {
      this.targetUserName=subitem.name
    },
    currentTime() {
      let date = new Date();
